...
5. Module-level `//!` docs should be offer-first: context lead, guide section with example,
   then Concern→API table when useful. Do not add `# Owns` / `# Does not own` inventories unless
   a product-line distinction is required (Soliton has no such case today).
6. Keep crate-root `# Features` capability bullets, module guide sections, and
   [`soliton/examples/README.md`](soliton/examples/README.md) discoverable: `process_host` is the
   copy-paste host wire-up; `hmac_health_host` is auth smoke.
7. Run the verification block in [`docs/VERIFICATION.md`](docs/VERIFICATION.md) before opening a PR.

### Style

- Organize crate-root docs by **Features** capability bullets and **Getting started** guides.
